# CFP Exam Coverage Map

**Last Updated**: October 18, 2025
**Days Until Exam**: 19 days (November 6, 2025)

This document maps your learning progress to the official CFP exam topic outline, showing which areas you've mastered, which have gaps, and which haven't been covered yet.

---

## Overall Progress by Domain

| Domain | Weight | Topics Covered | Status | Priority |
|--------|--------|----------------|--------|----------|
| **A. Professional Conduct & Regulation** | 8% | 0/6 topics | ⚪ Not Started | Medium |
| **B. General Principles** | 15% | 1/10 topics | 🟡 Started (10%) | High |
| **C. Risk Management & Insurance** | 11% | 4/10 topics | 🟡 In Progress (40%) | Medium |
| **D. Investment Planning** | 17% | 4/9 topics | 🟡 In Progress (44%) | High |
| **E. Tax Planning** | 14% | 6/8 topics | 🟢 Strong (75%) | Medium |
| **F. Retirement Savings & Income** | 18% | 8/10 topics | 🟢 Strong (80%) | **HIGHEST** |
| **G. Estate Planning** | 10% | 3/14 topics | 🟡 In Progress (21%) | Medium |
| **H. Psychology of Financial Planning** | 7% | 0/6 topics | ⚪ Not Started | Low |

**Overall Coverage**: 26/73 topics = **36% of exam topics covered**

---

## A. Professional Conduct and Regulation (8%)

### ⚪ Not Covered Yet (0/6 topics)

- [ ] **A.1** CFP Board's Code of Ethics and Standards of Conduct
- [ ] **A.2** CFP Board's Procedural Rules
- [ ] **A.3** Function, purpose, and general structure of financial institutions
- [ ] **A.4** Financial services regulations and requirements
- [ ] **A.5** Consumer protection laws
- [ ] **A.6** Fiduciary standard and application

**Status**: Not started
**Priority**: Medium (8% of exam, but foundational knowledge)
**Recommendation**: Review in final 2 weeks before exam

---

## B. General Principles of Financial Planning (15%)

### 🟢 Mastered Topics (1/10)

- [x] **B.12** Time value of money concepts and calculations
  - **PV/FV calculations** (tested 2025-10-17) - High confidence
  - Perfect equation setup: FV = PV × (1 + r)^n
  - Compound interest understanding
  - **Note**: Student had prior knowledge of this topic

### ⚪ Not Covered Yet (9/10 topics)

- [ ] **B.7** Financial planning process
- [ ] **B.8** Financial statements
- [ ] **B.9** Cash flow management
- [ ] **B.10** Financing strategies and debt management
- [ ] **B.11** Economic concepts
- [ ] **B.13** Education needs analysis
- [ ] **B.14** Education savings vehicles
- [ ] **B.15** Education funding
- [ ] **B.16** Gift / income tax strategies

**Status**: 10% coverage - Just started
**Priority**: HIGH (15% of exam - 3rd highest weighted domain)
**Recommendation**: **Focus on B.7-B.11 next** (financial planning process, statements, cash flow, economic concepts)

---

## C. Risk Management and Insurance Planning (11%)

### 🟢 Mastered Topics (4/10)

- [x] **C.19** Health insurance and health care cost management
  - **Medicare Parts A/B/C/D** (2025-10-15) - Medium-High confidence
  - Medicare enrollment and penalties - Medium-High confidence
  - Medicare cost-sharing - Medium confidence
  - **Gap**: Cost calculations improving (was Medium, now Low severity)

- [x] **C.20** Disability income insurance (individual and group)
  - **Own occupation vs any occupation** (tested 2025-10-17) - Medium-High confidence
  - Premium differences (own occ 2x more expensive)
  - Benefit qualification rules
  - Modified own occupation (introduced)
  - Best for high-income professionals with specialized skills

- [x] **C.23** Life insurance (partial coverage)
  - **Life insurance beneficiary strategies** (2025-10-11) - High confidence
  - Probate vs non-probate implications
  - **Gap**: Types of life insurance not yet covered

- [x] **C.26** Insurance policy and company selection (partial)
  - **Homeowners Insurance** (2025-10-13) - Medium-High confidence
    - HO policy types (HO-2, HO-3, HO-4, HO-6)
    - Coverage A/B/C/D structure
    - Special limits (jewelry/furs $1,500, coins $200, etc.)
    - 80% coinsurance rule
  - **Auto Insurance PAP** (2025-10-15) - High confidence
    - Parts A/B/C/D structure
    - Liability split limits calculations
    - Collision vs comprehensive
    - Covered persons and rental car coverage

### ⚪ Not Covered Yet (6/10 topics)

- [ ] **C.17** Principles of risk and insurance
- [ ] **C.18** Analysis and evaluation of risk exposures
- [ ] **C.21** Long-term care insurance and long-term care planning
- [ ] **C.22** Qualified and Non-Qualified Annuities
- [ ] **C.24** Business owner insurance solutions
- [ ] **C.25** Insurance needs analysis

**Status**: 40% coverage
**Priority**: Medium (11% of exam)
**Recommendation**: Add long-term care insurance and annuities next; also need elimination/benefit periods for disability

---

## D. Investment Planning (17%)

### 🟢 Mastered Topics (4/9)

- [x] **D.28** Types of investment risk
  - **R-squared and risk decomposition** (2025-10-11) - High confidence
  - **Systematic vs unsystematic risk** (2025-10-11) - High confidence
  - Correlation and diversification implications
  - Formula: Unsystematic risk = 1 - R²

- [x] **D.34** Investment strategies (partial coverage)
  - **Short selling mechanics** (2025-10-11) - High confidence
  - **Options strategies - Put options** (2025-10-11) - High confidence
  - When to use puts vs short selling
  - Max loss calculations

### ⚪ Not Covered Yet (5/9 topics)

- [ ] **D.27** Characteristics, uses and taxation of investment vehicles
- [ ] **D.29** Market cycles
- [ ] **D.30** Quantitative investment concepts and measures of investment returns
- [ ] **D.31** Asset allocation and portfolio diversification
- [ ] **D.32** Bond and stock valuation concepts
- [ ] **D.33** Portfolio development and analysis
- [ ] **D.35** Alternative investments and liquidity risk

**Status**: 44% coverage (but only partial)
**Priority**: HIGH (17% of exam - 2nd highest weighted domain)
**Recommendation**: **Focus on D.30 (quantitative concepts) and D.31 (asset allocation) next**

---

## E. Tax Planning (14%)

### 🟢 Mastered Topics (6/8)

- [x] **E.36** Fundamental and current tax law
  - **Tax doctrines** (2025-10-11) - High confidence
    - Step transaction
    - Constructive receipt
    - Assignment of income
    - Tax-deferred exchange (1031)

- [x] **E.37** Income tax fundamentals and calculations
  - **Capital gains/losses** (2025-10-11) - High confidence
  - Netting rules (ST and LT)
  - Tax rates (0%/15%/20% for LTCG)
  - Perfect execution on calculations

- [x] **E.38** Characteristics and income taxation of business entities
  - ⚠️ **KNOWLEDGE GAP - HIGH PRIORITY**
  - **Section 179, MACRS, Depreciation** (identified 2025-10-11)
  - Status: Needs More Work
  - Note: Introduced when tired, needs dedicated fresh session

- [x] **E.40** Tax reduction/management techniques
  - **Traditional IRA contributions & deductions** (2025-10-11) - High confidence
  - **Roth IRA strategy** (2025-10-11) - High confidence
  - **QCD (Qualified Charitable Distributions)** (2025-10-11) - High confidence
  - Active participant phase-outs
  - Contribute vs deduct distinction

- [x] **E.41** Tax consequences of property transactions
  - **Passive activity losses** (2025-10-11) - High confidence
  - $25k exception and AGI phase-outs
  - Disposition rule (all suspended losses released)

- [x] **E.43** Charitable/philanthropic contributions and deductions
  - **QCD tax treatment** (2025-10-11) - High confidence
  - Exclusion from income vs deduction

### ⚪ Not Covered Yet (2/8 topics)

- [ ] **E.39** Income taxation of trusts and estates
- [ ] **E.42** Tax implications of special circumstances

**Status**: 75% coverage - STRONG
**Priority**: Medium overall, but **HIGH priority to fix E.38 business taxation gap**
**Recommendation**: **Dedicate next fresh session to business taxation (E.38)**

---

## F. Retirement Savings and Income Planning (18%) ⭐ HIGHEST WEIGHT

### 🟢 Mastered Topics (8/10)

- [x] **F.45** Social Security and Medicare planning

  **Social Security** - High confidence:
  - Filing timeline (early/FRA/delayed) - mastered
  - Earnings test (threshold, penalty calculation) - tested 2025-10-16, excellent
  - Spousal benefits (requirements, timing) - mastered
  - Taxation (combined income, thresholds) - Medium-High confidence
  - ⚠️ **Minor gap**: Early filing reduction % (thinks 5%, actually 6-7%)

  **Medicare** - High confidence (comprehensive deep dive 2025-10-18):
  - **Part A Hospital**: Days 1-60 (deductible $1,632), 61-90 ($408/day), 91-150 (60 lifetime reserve) ✓
  - **Part A SNF**: Days 1-20 (FREE), 21-100 ($204/day), requires 3-day hospital stay ✓
  - **Part B**: Doctor visits, outpatient, premium $174.70/month + IRMAA ✓
  - **Part C (Medicare Advantage)**: Private plans, lower cost, smaller network, has out-of-pocket max, extras (dental/vision) ✓
  - **Part D**: Prescription drugs, $7-$200+/month ✓
  - **MAPD**: Medicare Advantage + Part D combined ✓
  - **Medicare Supplements (Medigap)**: Higher cost (~$200-$300/month), full network, covers copays/deductibles, NO out-of-pocket max ✓
  - **MA vs Medigap tradeoffs**: Lower income→MA, higher income→Medigap, understands market dynamics ✓
  - **Enrollment periods**: IEP (7 months at 65), AEP (Oct 15-Dec 7), OEP (Jan 1-Mar 31 for MA only), SEP (qualifying events) ✓
  - **Agent commissions**: MA ~$600-700 first year, Medigap ~20% of premium, Traditional Medicare $0 ✓
  - **Coverage limits**: Medicare ends at 150 days hospital, 100 days SNF - neither Original nor Medigap extends beyond ✓

- [x] **F.47** Types of retirement plans
  - **Retirement plan types** (2025-10-13) - High confidence
    - 403(b), 457(b), 457(f)
    - Contribution limits ($23k + $7.5k)
  - **Keogh plans (HR-10)** (2025-10-13) - High confidence
    - 20% for self-employed
    - Separate from IRA limits

- [x] **F.48** Qualified plan rules and options
  - **Retirement plan classifications** (2025-10-11) - High confidence
    - DC vs DB plans
    - Pension vs profit-sharing
    - Cash balance, target benefit, money purchase pension
  - **Nondiscrimination testing** (2025-10-11) - High confidence
    - HCE definition
    - Ratio percentage test
    - Average benefits test

- [x] **F.51** Distribution rules and taxation
  - **RMD rules** (tested 2025-10-17) - High confidence
    - RMD age requirements: 73 (born 1951-1959), 75 (born 1960+)
    - RMD calculation: Account balance ÷ life expectancy factor
    - Perfect calculation: $500k ÷ 26.5 = $18,868
    - April 1 delay exception for first RMD only
    - 25% penalty for missed RMD (reducible to 10% if corrected)
    - Roth IRAs: No RMD during owner's lifetime
  - **Gap**: Early withdrawal exceptions and 72(t) not yet covered

### ⚪ Not Covered Yet (2/10 topics)

- [ ] **F.44** Retirement needs analysis
- [ ] **F.46** Eldercare and special needs planning
- [ ] **F.49** Non-qualified plan rules and options
- [ ] **F.50** Key factors affecting plan selection for businesses
- [ ] **F.52** Retirement income and distribution strategies
- [ ] **F.53** Business succession planning

**Status**: 80% coverage - EXCELLENT foundation
**Priority**: **HIGHEST** (18% of exam - most heavily weighted domain)
**Recommendation**: **Add early withdrawal exceptions (F.51 continued) and retirement income strategies (F.52)**

---

## G. Estate Planning (10%)

### 🟢 Mastered Topics (3/14)

- [x] **G.54** Property titling and beneficiary designations
  - **Probate vs non-probate assets** (2025-10-11) - High confidence
  - **Property ownership types** (2025-10-11) - High confidence
    - JTWROS (avoids probate)
    - Tenants in common (goes through probate)
  - **Life insurance beneficiary strategies** (2025-10-11) - High confidence
    - To person vs to estate
    - Estate liquidity planning

### ⚪ Not Covered Yet (11/14 topics)

- [ ] **G.55** Strategies to transfer property
- [ ] **G.56** Estate and incapacity planning documents
- [ ] **G.57** Gift, estate, and GST tax compliance and calculation
- [ ] **G.58** Sources for estate liquidity
- [ ] **G.59** Types, features, and taxation of trusts
- [ ] **G.60** Marital deduction
- [ ] **G.61** Intra-family and other business transfer techniques
- [ ] **G.62** Postmortem estate planning techniques
- [ ] **G.63** Planning for divorce, unmarried couples and other special circumstances
- [ ] **G.64** Planning for special needs and circumstances

**Status**: 21% coverage - NEEDS WORK
**Priority**: Medium (10% of exam)
**Recommendation**: Cover trusts (G.59) and gift/estate tax (G.57) in next 1-2 weeks

---

## H. Psychology of Financial Planning (7%)

### ⚪ Not Covered Yet (0/6 topics)

- [ ] **H.65** Client and planner attitudes, values, biases
- [ ] **H.66** Behavioral finance
- [ ] **H.67** Sources of money conflict
- [ ] **H.68** Principles of counseling
- [ ] **H.69** General principles of effective communication
- [ ] **H.70** Crisis events with severe consequences

**Status**: Not started
**Priority**: Low (7% of exam, lowest weight)
**Recommendation**: Brief review in final week before exam

---

## Priority Action Plan (Next 20 Days)

### 🔴 URGENT - Address Immediately (Days 1-7)

1. **E.38 Business Taxation** ⚠️ HIGH PRIORITY GAP
   - Section 179 expensing
   - MACRS depreciation
   - Mid-quarter convention
   - Start-up cost rules
   - Section 1245 recapture

2. **B.12 Time Value of Money** ⚠️ CRITICAL FOUNDATION
   - PV/FV calculations
   - Annuities
   - NPV/IRR
   - (Required for many other domains)

3. **F.51 Distribution Rules and Taxation**
   - RMD rules (ages 73 vs 75)
   - Early withdrawal exceptions
   - 72(t) distributions
   - Required beginning date

### 🟡 HIGH PRIORITY - Cover in Week 2 (Days 8-14)

4. **B.7-B.11 General Principles**
   - Financial planning process
   - Financial statements
   - Cash flow management
   - Economic concepts

5. **D.30-D.31 Investment Quantitative Concepts**
   - Beta, alpha, Sharpe ratio
   - Asset allocation
   - Portfolio diversification

6. **G.57, G.59 Estate Planning Core**
   - Gift/estate/GST tax
   - Trusts (types and taxation)

### 🟢 MEDIUM PRIORITY - Week 3 (Days 15-18)

7. **C.20-C.21 Insurance (Disability, LTC)**
8. **F.52 Retirement Income Strategies**
9. **Review all mastered topics**

### ⚪ LOW PRIORITY - Final Days (Days 19-20)

10. **A.1-A.6 Professional Conduct** (quick review)
11. **H.65-H.70 Psychology** (quick review)

---

## Current Knowledge Gaps (Requires Attention)

### 🔴 High Severity

1. **E.38 Business Taxation** (Section 179, MACRS, Depreciation)
   - Status: Needs More Work
   - Impact: Critical for Tax Planning domain (14% of exam)
   - Action: Dedicate fresh session ASAP

### 🟢 Low Severity (Minor Refinement)

2. **F.45 Social Security Early Filing Reduction %**
   - Status: Needs Minor Refinement
   - Issue: Thinks ~5% per year, actually ~6-7% per year
   - Action: Quick memorization fix

### ✅ Recently Resolved

3. **F.45 Medicare Cost Calculations and Comprehensive Understanding**
   - Date Resolved: 2025-10-18
   - Previous Status: Medium severity gap with day ranges and cost-sharing confusion
   - Resolution: Comprehensive deep dive with online research
   - Now mastered: Hospital stay (1-60, 61-90, 91-150 days), SNF (1-20 free, 21-100 copay), MA vs Medigap tradeoffs, enrollment periods, agent commissions, coverage limits
   - Confidence: High ✓

---

## Strengths to Leverage

✅ **Tax Planning** (75% covered) - Build on this strength
✅ **Retirement Planning** (70% covered) - Already strong in highest-weighted domain
✅ **Quick learner** - Excellent at calculations once formula is understood
✅ **Strong retention** - Demonstrated across multiple sessions
✅ **Self-awareness** - Knows when tired, asks clarifying questions

---

## Study Strategy Recommendations

### By Exam Weight (Focus on highest-impact topics first)
1. **Retirement (18%)** - 70% done, finish strong ✓
2. **Investment (17%)** - 44% done, needs more work
3. **General Principles (15%)** - 0% done, **URGENT**
4. **Tax (14%)** - 75% done, fix business tax gap ✓
5. **Insurance (11%)** - 30% done, add disability/LTC
6. **Estate (10%)** - 21% done, add trusts/gift tax
7. **Professional Conduct (8%)** - Quick review later
8. **Psychology (7%)** - Quick review later

### Time Allocation (20 days remaining)
- **Days 1-7**: Business tax, Time value of money, RMDs
- **Days 8-14**: General principles, Investment quant, Estate trusts/tax
- **Days 15-18**: Insurance gaps, Retirement income, Review mastered
- **Days 19-20**: Professional conduct, Psychology, Final review

---

**Last Updated**: October 17, 2025
**Next Review**: After each study session
**Goal**: Cover all high-priority gaps by Day 14, review Days 15-20
